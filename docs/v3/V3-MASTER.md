...
| **V3-116** | Rentabilidad del producto: inversión, recuperación y proyección por producto | Inventario | Básico | en implementación | — | BE `158` |
| **V3-117** | Ranking de rentabilidad entre productos | Sugerida | Pro | idea | V3-116 | §9 |
| **V3-118** | Ficha de rentabilidad exportable (PDF/Excel) | Sugerida | Pro | idea | V3-116, exports | §9 |
| **V3-119** | Backfill del costeo histórico + reponer las vistas de costeo retiradas | Inventario | — | especificada | — | [pendientes-costeo](../pendientes-costeo.md) |
| **V3-120** | Contraste del texto secundario en modo oscuro (`--muted-foreground`) | Transversal | — | idea | — | §9 |
...
| **V3-104** | **Pronóstico de demanda** — velocidad de venta y días-hasta-agotarse con media móvil; sugiere cantidad de compra. Su consumidor natural es la vista de V3-116, que podría cerrar con un "a este ritmo recuperas la inversión en X días"; se dejó fuera de su v1 a propósito, porque con ventas irregulares —lo normal en un negocio pequeño— una extrapolación lineal da fechas absurdas y quema la credibilidad del resto de la pantalla. | Evita roturas sin ML | M | V3-092 |
...
| **V3-117** | **Ranking de rentabilidad entre productos** — la pregunta que sigue a V3-116 ("¿cuál me da más?"). Es un endpoint **por negocio**, no por producto, así que su casa es Analíticas y no la ficha de un producto. | Decide qué reponer y qué dejar de comprar | M | V3-116 |
| **V3-118** | **Ficha de rentabilidad exportable** — el PDF/Excel de la vista de V3-116, para enseñárselo a un socio o a un contador. Encaja con el patrón de exports ya gateado por la llave `exports`. | Saca la cifra de la pantalla sin copiarla a mano | M | V3-116 |

| **V3-120** | **Contraste del texto secundario en modo oscuro** — en el bloque oscuro de `globals.css`, `--muted-foreground` (`215 12% 55%`) sobre el fondo de las tarjetas (`215 25% 10%`) da ≈4,7:1: pasa el mínimo AA por poco y en `text-xs` se lee con esfuerzo. Detectado al revisar la vista de V3-116, pero afecta a **todas** las pantallas, porque es el patrón del proyecto entero para etiquetas, subtítulos y notas al pie. El arreglo es una línea —subirlo a algo como `215 14% 68%`, que lo lleva a ≈8:1 sin perder la jerarquía frente al texto principal—; el trabajo real es la revisión visual de las pantallas después. | Legibilidad de media aplicación, y accesibilidad AA con holgura | S (el cambio) + M (la revisión) | — |
...
| 2026-08-24 | Alta de **V3-120** (contraste del texto secundario en modo oscuro), `idea`. Salió de una queja concreta sobre los popovers de la vista de V3-116 —donde estaba además sobrescrito el color que el sistema ya daba bien, y quedó corregido— pero el token de fondo es del proyecto entero: el mismo gris se usa en todas las tarjetas de todas las pantallas. Se registra en vez de cambiarlo en caliente porque una línea en `globals.css` repinta la aplicación completa y hay que revisarla. |
| 2026-08-24 | Alta de **V3-116** (rentabilidad del producto), `en implementación`, y de **V3-117**, **V3-118** y **V3-119** como trabajo derivado. V3-116 cruza por primera vez el desembolso acumulado de un producto con lo cobrado: las vistas de costeo existentes respondían "qué hay en el almacén" y "qué margen dejó cada compra", pero ninguna "¿esto ya se pagó solo?". Cuatro decisiones de alcance cerradas: nombre **Rentabilidad del producto** (ruta `/dashboard/business/inventory/profitability`, con los dos accesos de consulta agrupados en un popover en la tabla de stock); **doble cifra** de recuperación —lo facturado y lo cobrado de verdad, prorrateando el pago de cada venta entre sus líneas—; **sin gating de plan**, como el historial y las tarjetas de costeo; y tres extras dentro de la v1 —unidades que faltan por vender, variación de costo entre lotes y alerta de margen negativo antes de vender—. Fuera de alcance por decisión: rotación y proyección de días (anotada en V3-104), ranking entre productos (V3-117) y exportar la ficha (V3-118). Contrato backend en [migration_doc/158](../../../psearch-back/src/v2/migration_doc/158-product-investment-status.md). |
